...
jQuery(document).ready(function () {
    ...
    var slimmscrollNav = $(window).height() - 94;

    $('.accordion-menu.responsive-menu').slimScroll({
        height: slimmscrollNav
    });
    ...
    function catagoryBind() {
        ...
        $('.dropdown-special.catagory-btn').autocomplete({
            source: Countries,
            minLength: 1,
            scroll: true
        }).on('click', function () {
            $('#categoty_list').autocomplete("search", "");
        });
    }
    catagoryBind();

    function locationBind() {
        var CityInput = document.getElementById('city_location');
        var CityDropdownOptions = {
            types: ['(cities)'],
            componentRestrictions: { country: "ba" }
        };
        var autocomplete = new google.maps.places.Autocomplete(CityInput, CityDropdownOptions);
        ...
        // $('.dropdown-special.location-btn').autocomplete({
        //     source: Countries,
        //     minLength: 0,
        //     scroll: true
        // }).on('click', function () {
        //     $('#city_location').autocomplete("search", "");
        // });
    }
    locationBind();
    /*--------------------------------------------
